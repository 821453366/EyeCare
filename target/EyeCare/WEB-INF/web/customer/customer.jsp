...
<script id="list-tpl" type="text/html">
    ...
    <tr>
        <td>{{ index+1}}</td>
        <th>{{item.username == undefined ? "暂无" : item.username}}</th>
        <th>{{item.sex == undefined ? "暂无" : item.sex}}</th>
        <th>{{item.age == undefined ? "暂无" : item.age}}</th>
        <th>{{item.phone == undefined ? "暂无" : item.phone }}</th>
        <th>{{item.email == undefined ? "暂无" : item.email}}</th>
        <%--<td>--%>
    ...
    layui.use(['jquery', 'layer', 'element', 'laypage', 'form', 'laytpl', 'tree','upload'], function () {
        ...
        cl = {
            ...
            list: function () {
                ...
                $.post("${pageContext.request.contextPath}/customer/findUser", {
            ...
            preview: function (id) {
                $.post("${pageContext.request.contextPath}/customer/userInfo",
                    {id:id},
                    function (data) {
            ...
            $.post("${pageContext.request.contextPath}/customer/userInfo",{id:id},
                function (data) {
                    let user = data.user[0];
            ...
            $.post("${pageContext.request.contextPath}/customer/updateInfoAjax",
                eyeEmp,
                function (data) {
                    layer.msg(data.msg);
                }
            ...
            addUserAjax: function () {
                ...
                $.post("${pageContext.request.contextPath}/customer/addUser", eyeEmp, function (data) {
            ...
            delete:function (id) {
                layer.confirm('确定删除？', {icon: 3, title: '提示'}, function (index) {
                    layer.close(index);
                    $.post("${pageContext.request.contextPath}/customer/deleteUser", {id: id},
                        function (data) {
                            layer.msg(data.msg, {time: 500});
                            if (data.result) {
